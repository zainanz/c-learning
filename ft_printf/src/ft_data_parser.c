...
int	ft_set_precision_width(char *format, int i, t_print_data *data)
{
	if (format[i] == '.')
		data->precision = ft_atoi(&format[++i]);
	else
		data->width = ft_atoi(&format[i]);
	i += ft_skip_digits(&format[i]);
	return (i);
}
...
int	ft_data_parse(char *format, t_print_data *data)
{
	...
	while (format[i])
	{
		if (format[i] == '-')
			...
			i = ft_set_precision_width(format, i, data);
